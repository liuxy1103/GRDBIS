...
class cross_image_memory(nn.Module):
    ...
    def __init__(self, memory_size, contrast_size, t_channels, img_size):
        ...
        self.feature_size_z, self.feature_size_h, self.feature_size_w = 25, img_size, img_size
        self.register_buffer("teacher_feature_queue", torch.randn(self.memory_size, self.dim, self.feature_size_z, self.feature_size_h, self.feature_size_w)) # MxCxHxW
        self.register_buffer("teacher_mask_queue", torch.randn(self.memory_size,1, self.feature_size_z, self.feature_size_h, self.feature_size_w)) # Mx1xZxHxW
    ...
    def _dequeue_and_enqueue(self, keys, labels): # keys:#BxCxHxW  labels: #Bx1xHxW
        ...
        batch_size, feat_dim,Z, H, W = keys.size()
        
        for bs in range(batch_size):
            this_feat = keys[bs]  #CxZxHxW
            this_label = labels[bs]#1xZxHxW
    ...
    def _sample_negative(self, index):
        cache_size, feat_size, Z, H, W = self.teacher_feature_queue.shape  # MxCxZxHxW
        contrast_size = index.size(0)
        if contrast_size > self.queue_number:
            contrast_size = self.queue_number
            index = index[:contrast_size]
        # X_ = torch.zeros((contrast_size, feat_size, Z, H, W)).float().cuda()
        # y_ = torch.zeros((contrast_size, 1, Z, H, W)).float().cuda()
    ...
    def forward(self, t_feats, labels):  #feats: BxCxHxW, labels: Bx1xHxW
        ...
        queue_size, _, _, _, _ = self.teacher_feature_queue.shape
...
def calculate_CI_affinity_loss(this_embedding, embedding, embedding_T,loss_type='KL'):
    M,C,Z,H,W = this_embedding.shape
    B,C,Z,H,W = embedding.shape
    ...
    for i in range(M):
        for j in range(B):
            avg_pool = nn.AvgPool3d(kernel_size=(3, 7, 7), stride=(3, 7, 7), padding=0, ceil_mode=True)
            ...
            if loss_type=='KL':
                ...
                s1 = torch.prod(torch.tensor(s_sim_map.size()[-2:]).float())
                norm_term = (s1).cuda()
        ...
        for x_T,x_S in zip(h_list_T_b,h_list_b):
            for h_list_this_b in h_list_this:
                for x_this in h_list_this_b:
                    num_inter = x_T.shape[0]
                    # num_intra = x_this.shape[0] + num_inter
                    ...
                    if if_edge_relation:
                        if loss_type=='KL':
                            ...
                            edge_T = edge_T*mask
                            edge_S = edge_S*mask
                            # edge_T[:num_inter,:num_inter]=0
                            # edge_T[num_inter:,num_inter:]=0                            
                            # edge_S[:num_inter,:num_inter]=0
                            # edge_S[num_inter:,num_inter:]=0
                            if if_neighbor:
                                loss_edge = torch.sum(torch.abs(edge_T-edge_S)**2)/torch.sum(adj == 1)*edge_weight
                            else:
                                loss_edge = torch.sum(torch.abs(edge_T - edge_S)**2) / num_inter/ (x_T.shape[0]-num_inter)*edge_weight
