                    ...
                    if if_edge_relation:
                        if loss_type=='KL':
                            ...
                            if if_neighbor:
                                ...
                                loss_edge = torch.sum(torch.abs(dis_edge)**2) / num_inter/ (x_T.shape[0]-num_inter)*edge_weight
    ...
    for h_list_T_b,h_list_b,edge_list_b in zip(h_list_T,h_list,edge_list):
        for x_T,x_S,edge in zip(h_list_T_b,h_list_b,edge_list_b):
            ...
            if if_edge_relation:
                ...
                if if_neighbor:
                    loss_edge = torch.sum(torch.abs(dis_edge))/torch.sum(adj == 1)*edge_weight
                else:
                    loss_edge = torch.sum(torch.abs(dis_edge)) / (x_T.shape[0])/ (x_T.shape[0])*edge_weight
                # loss_edge = torch.sum(torch.abs(edge_T - edge_S)) / torch.sum(adj == 1)
                loss_all += loss_edge
