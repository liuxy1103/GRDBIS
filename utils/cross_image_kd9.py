...
def pair_wise_sim_map(fea_0, fea_1):
    ...
    sim_map_0_1 = torch.mm(fea_0, fea_1.transpose(0, 1))  #(N,F)x(F,N)
    return sim_map_0_1
                ...
                for x_this in h_list_this_b:
                    ...
                    if if_edge_relation:
                        if loss_type=='KL':
                            ...
                            norm_T = torch.norm(edge_T.reshape(-1),p=2)
                            norm_S = torch.norm(edge_S.reshape(-1),p=2)
                            edge_T = edge_T/norm_T
                            edge_S = edge_S/norm_S
